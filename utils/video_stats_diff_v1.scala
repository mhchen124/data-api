...
var modified  = diffed.map( x => {
	...
	var reactNew:Reaction = null
        ...
        try {
            //val jsonOld = parse(oldVal)
            //val reactOld = jsonOld.extract[Reaction]
            //val jsonNew = parse(newVal)
            //val reactNew = jsonNew.extract[Reaction]
            //val diff = reactNew - reactOld
            //colDiff = """{"love":""" + diff.love + ""","haha":""" + diff.haha + ""","like":""" + diff.like + ""","sorry":""" + diff.sorry + ""","anger":""" + diff.anger + ""","wow":""" + diff.wow + "}"
	    //val items = jsonNew.extract[Map[String, Int]].map(x => "{" + x._1 + x._2.toString + "}") 
